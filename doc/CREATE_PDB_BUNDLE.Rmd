...
The reference-draw `write_pdb()` method also computes and writes the supported
summary statistics (`mean_value` and `sd`) using their existing constructors
and writer methods. You do not need to write those separately when saving
reference draws. Data, model, reference-draw, and summary-statistic payloads
are written with their info files; summary statistics are saved under
...
`mean_value` contains `names`, `mean_value`, and `mcse_mean`; `sd` contains
`names`, `sd`, and `mcse_sd`. Their info JSON uses the reference-draw metadata
